...
async function fetchProduct(props: { productID: string }) {
    const response = await fetch(`${pathname}/api/product-get?id=${props.productID}`);
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        ...
                        <div className="md:w-1/2 flex flex-col justify-center p-2 gap-5">
                            ...
                            <div className="flex flex-col gap-2">
                                <h2 className="text-sm uppercase text-blue-600 font-bold mb-1">{product.sellerName}</h2>
                                <h1 className="text-3xl font-bold mb-1">{product.productName}</h1>
                            ...
                            <div>
                                <div className="flex items-center">
                                    <span className="text-3xl font-bold mr-4">&#8377; {product.discountPrice}</span>
                                    <span className="bg-blue-100 text-blue-500 px-2 py-1 rounded-md font-bold">
                                        {Math.round(((product.price - product.discountPrice) / product.price) * 100)}% off
                                    </span>

                                </div>
                                <div className="text-slate-400 line-through">&#8377; {product.price}</div>
                            </div>
                            ...
                            <div className="flex flex-row items-center bg-blue-100 px-3 py-2.5 rounded-2xl shadow-md text-blue-500 gap-2 w-fit max-w-md hover:scale-[1.02] cursor-pointer duration-300">
                                ...
                                <div className="flex flex-row gap-0">
                                    <span className="text-sm font-bold">{product.totalAvailable} in stock</span>
                                </div>
                            </div>
