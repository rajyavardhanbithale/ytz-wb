...
export default function Sidebar() {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`z-[100] fixed top-0 left-0 h-full bg-blue-950 text-white flex flex-col w-64 shadow-lg transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
            >
                ...
                <nav className="flex flex-col flex-1 justify-center gap-5 p-2">
                    {options.map((item, idx) => (
                        <button
                            ...
                            onClick={() => dispatch(selectOperation(item))}
                        >
                            {item}
                        </button>
                    ))}
