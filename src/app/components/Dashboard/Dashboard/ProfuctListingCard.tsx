    ...
    return (
        <div className="flex flex-col justify-between bg-white shadow-lg rounded-xl overflow-hidden max-w-md mx-auto my-6 hover:scale-[1.02] transition-transform duration-300">
            ...
            <div className="p-5 rounded-2xl bg-white">
                <h2 className="text-3xl font-semibold mb-2 text-gray-800">{product.productName}</h2>
                <p className="text-sm text-gray-500 mb-3">Seller: {product.sellerName}</p>
                <p className="text-gray-700 mb-4">{product.description}</p>
                <div className="flex items-baseline mb-4">
                    <span className="text-2xl font-bold text-gray-900 mr-2">₹{product.discountPrice}</span>
                ...
                <p className={`text-sm font-semibold mb-2 ${product.inStock ? 'text-green-600' : 'text-red-600'}`}>
                    In Stock: {product.inStock ? 'Yes' : 'No'}
                </p>
                <p className="text-sm text-gray-500">Available Stock: {product.totalAvailable}</p>
            </div>
